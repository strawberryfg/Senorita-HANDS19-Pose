...
namespace caffe {
	...
	void DeepHandModelGen3DSkeletonMapPerChannelV2Layer<Dtype>::LayerSetUp(
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelV2Layer<Dtype>::Reshape(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelV2Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
				...
				for (int k = 0; k < depth_dims_; k++)
				{
					for (int row = 0; row < map_size_; row++)
					{
						for (int col = 0; col < map_size_; col++)
						{
							...
							int u = bones[j][0];
							int v = bones[j][1];
							double x1 = 0.0;// gt_joint_2d_data[Jid + u * 2];
							double x2 = gt_joint_2d_data[Jid + v * 2] - gt_joint_2d_data[Jid + u * 2];
							double y1 = 0.0;// gt_joint_2d_data[Jid + u * 2 + 1];
							double y2 = gt_joint_2d_data[Jid + v * 2 + 1] - gt_joint_2d_data[Jid + u * 2 + 1];
							double z1 = 0.0;// gt_depth_data[Did + u];
							double z2 = gt_depth_data[Did + v] - gt_depth_data[Did + u];
							if (!dot_product_)
							{
															...
															//next step: consider occlusion
								{
									if (flag_x_a == flag_x_b && flag_y_a == flag_y_b && flag_z_a == flag_z_b)
									{
										...
										{
											top_data[Tid + j * depth_dims_ * map_size_ * map_size_ + k * map_size_ * map_size_ + row * map_size_ + col] = 1.0; //voxel occupied
										}
									}
								}
							}
							else
							{
								...
								if (r > 1e-6 && r <= 1.0)
								{
									...
									double th = this->layer_param_.output_skeleton_param().pixel_value_threshold() / 255.0;
									if (prob > th) top_data[Tid + j * depth_dims_ * map_size_ * map_size_ + k * map_size_ * map_size_ + row * map_size_ + col] = 1.0;
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelV2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
							...
							for (int col = 0; col < map_size_; col++)
							{
								...
								if (dot_product_) //exponential representation
								{
									...
									if (r > 1e-6 && r <= 1.0) //within segment
									{
										double prob = exp(-gamma_ * d);
										//top_data[Tid + j * depth_dims_ * map_size_ * map_size_ + k * map_size_ * map_size_ + row * map_size_ + col] = prob;
										double top_diff_value = top_diff[Tid + j * depth_dims_ * map_size_ * map_size_ + k * map_size_ * map_size_ + row * map_size_ + col];
...
#ifdef CPU_ONLY
	STUB_GPU(DeepHandModelGen3DSkeletonMapPerChannelV2Layer);
#endif

	INSTANTIATE_CLASS(DeepHandModelGen3DSkeletonMapPerChannelV2Layer);
	REGISTER_LAYER_CLASS(DeepHandModelGen3DSkeletonMapPerChannelV2);
}
