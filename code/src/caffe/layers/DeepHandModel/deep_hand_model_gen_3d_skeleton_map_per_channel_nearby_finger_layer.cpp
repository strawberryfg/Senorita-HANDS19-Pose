...
namespace caffe {
	...
	void DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer<Dtype>::LayerSetUp(
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer<Dtype>::Reshape(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

		vector<int> top_shape;

		top_shape.push_back((bottom[0]->shape())[0]);
		top_shape.push_back((BoneNum + 64) * depth_dims_);
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		for (int t = 0; t < batSize; t++) 
		{
			...
			int Did = t * JointNum;
			int Tid = t * (BoneNum + 64) * depth_dims_ * map_size_ * map_size_;
			//for (int j = 0; j < BoneNum; j++) 
			//clear
			for (int j = 0; j < (BoneNum + 64) * depth_dims_ * map_size_ * map_size_; j++) top_data[Tid + j] = 0.0;

			//for each voxel find the closest bone
			for (int j = 0; j < BoneNum + 64; j++)
			{
	...
	template <typename Dtype>
	void DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
	STUB_GPU(DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer);
#endif

	INSTANTIATE_CLASS(DeepHandModelGen3DSkeletonMapPerChannelNearbyFingerLayer);
	REGISTER_LAYER_CLASS(DeepHandModelGen3DSkeletonMapPerChannelNearbyFinger);
}
