...
namespace caffe {
	...
	void GenHeatmapAllChannelsLayer<Dtype>::LayerSetUp(
		...
		//01/11/2019 added if 1-row heatmap
		use_one_row_ = this->layer_param_.gen_heatmap_all_channels_param().use_one_row();
	...
	void GenHeatmapAllChannelsLayer<Dtype>::Reshape(
		...
		if (use_one_row_) top_shape.push_back(1);
		else top_shape.push_back(gen_size_);
				...
				else //one row heatmap
				{
					for (int row = 0; row < 1; row++)
					{
						for (int col = 0; col < gen_size_; col++)
						{
							...
							{
								float gt_x = bottom_data[Bid + channel * 2], gt_y = bottom_data[Bid + channel * 2 + 1];
								//gt_y is not used below
								float t = exp(-1.0 / (2.0 * (render_sigma_ * render_sigma_)) * (pow(col / float(gen_size_) - gt_x, 2)));
								if (all_one_)
								{
									float dist = sqrt(pow(col / float(gen_size_) - gt_x, 2));
									if (render_sigma_ - dist > 1e-6)
									{
										//within radius
										top_data[Tid + channel * gen_size_ * gen_size_ + row * gen_size_ + col] = 1.0;
