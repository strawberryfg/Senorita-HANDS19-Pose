...
std::vector<int> missing_index_arr;
namespace caffe {
	...
	void GenRandIndexLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (strcmp(missing_index_file_.c_str(), "D:\\") != 0) //!="D:\\"
		{
			...
			{
				int t_id;
				fscanf(fin_missing_index, "%d", &t_id);
				missing_index_arr.push_back(t_id);
			}
		...
		for (int t = 0; t < batSize; t++)
		{
			...
			else
			{
				...
				rand_index = distribution_rand(generator_rand) * ((int)index_upper_bound_ - (int)index_lower_bound_ + 1) + index_lower_bound_;
			...
			if (strcmp(missing_index_file_.c_str(), "D:\\") != 0) //!="D:\\"
			{
				...
				{
					bool t_mark = false;
					for (int i = 0; i < num_of_missing_; i++) if (rand_index == missing_index_arr[i]) { t_mark = true; break; }
