    ...
    void DeepHandModelCubiodIntoGlobalV2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		...
		for (int t = 0; t < batSize; t++) {
			...
			for (int i = 0; i < joint_num_; i++) {
				...
				//top_data[Tid + i * 3 + 1] = global_y;
				//top_data[Tid + i * 3 + 2] = global_z;
				bottom_diff[Bid + i * 3] = top_diff[Tid + i * 3] * 1.0 / 2.0 * (x_ub - x_lb);
				bottom_diff[Bid + i * 3 + 1] = top_diff[Tid + i * 3 + 1] * -1.0 / 2.0 * (y_ub - y_lb);
				bottom_diff[Bid + i * 3 + 2] = top_diff[Tid + i * 3 + 2] * -1.0 / 2.0 * (z_ub - z_lb);
			}
